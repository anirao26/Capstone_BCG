...
class SelPredictor(nn.Module):
    def __init__(self, N_word, N_h, N_depth, max_tok_num, use_ca):
        ...
        self.sel_lstm = nn.LSTM(input_size=int(N_word), hidden_size=int(N_h/2),
        ...
        self.sel_col_name_enc = nn.LSTM(input_size=int(N_word), hidden_size=int(N_h/2),
                num_layers=int(N_depth), batch_first=True,
                dropout=0.3, bidirectional=True)
    ...
    def forward(self, x_emb_var, x_len, col_inp_var,
        ...
        if self.use_ca:
            ...
            for idx, num in enumerate(x_len):
                if num < max_x_len:
                    att_val[idx, :, num:] = -100
            att = self.softmax(att_val.view((-1, int(max_x_len)))).view(
                    B, -1, int(max_x_len))
